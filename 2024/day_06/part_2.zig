...
const WalkMap = [SIZE.h][SIZE.w]?Dir;
...
pub fn main() !void {
    ...
    const walkmap = try GPA.create(WalkMap);
    ...
    while (true) {
        // marking location
        if (walkmap[guard.r][guard.c]) |d| if (d == guard.d.right()) {
            std.log.debug("{},{}", .{ guard.r, guard.c });
            count += 1;
        };
        walkmap[guard.r][guard.c] = guard.d;
